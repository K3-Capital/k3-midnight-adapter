...
contract BlueMidnightAdapter is IBlueMidnightAdapter {
        ...
        returns (bytes32[] memory ids, int256 change)
    {
        ...
        if (assets != 0) {
            (withdrawn, burnedShares) = IMorpho(morphoBlue).withdraw(market, assets, 0, address(this), parentVault);
    ...
    function blueAvailableLiquidity() public view returns (uint256) {
        ...
        return cash < market.totalSupplyAssets ? cash : market.totalSupplyAssets;
    }
    ...
    function _exposureCapacity() internal view returns (uint256) {
        ...
        if (globalExposureCap != 0) capacity = globalExposureCap;
        if (targetExposureCap != 0 && targetExposureCap < capacity) capacity = targetExposureCap;
